...
def import_geo_data():
    ...
    # Path to Kaggle downloaded files (configurable via KAGGLE_GEO_DIR)
    default_dir = os.path.join(seeds_dir, "geo_dataset")
    dataset_dir = os.getenv("KAGGLE_GEO_DIR", default_dir)
    ...
    for path in [countries_path, states_path, cities_path]:
        if not os.path.exists(path):
            print(f"Error: Required dataset file not found at: {path}. Set KAGGLE_GEO_DIR or place json files in {dataset_dir}")
            return
